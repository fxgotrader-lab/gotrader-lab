...
const outRoot = path.join(projectRoot, ".gotrader", "cisd-performance-test");
...
const requestedLookbackDays = Number(process.env.CISD_LOOKBACK_DAYS || 90);
const timeframes = (process.env.CISD_TIMEFRAMES || "5m,15m").split(",").map((item) => item.trim()).filter(Boolean);
const chunkDays = Number(process.env.CISD_CHUNK_DAYS || 7);
const limitPerChunk = Math.max(1, Math.min(5000, Number(process.env.CISD_CHUNK_LIMIT || 5000)));
const timeoutMs = Number(process.env.MT5_READONLY_TEST_TIMEOUT_MS || 12000);
const sliceCandlesByTimeframe = { "5m": Number(process.env.CISD_5M_SLICE_CANDLES || 80), "15m": Number(process.env.CISD_15M_SLICE_CANDLES || 64) };
const lookaheadByTimeframe = { "5m": Number(process.env.CISD_5M_LOOKAHEAD_CANDLES || 72), "15m": Number(process.env.CISD_15M_LOOKAHEAD_CANDLES || 48) };
...
const normalizeMt5Candles = ({ candles = [], timeframe }) => {
  ...
  return candles
    ...
    .map((candle, index) => ({
      id: `mt5_cisd_${brokerSymbol}_${timeframe}_${parseCandleTime(candle)}_${index}`,
...
const sessionBucket = (minuteOfDay) => {
  if (minuteOfDay >= 570 && minuteOfDay < 630) return "session_open";
  if (minuteOfDay >= 570 && minuteOfDay < 960) return "rth_non_open";
  return "outside_rth";
};
...
const compactCandidate = ({ candidate, timeframe, signalIndex, outcome, tradingDate, session }) => ({
  key: [timeframe, candidate.cisdCandle?.timestamp, candidate.side, candidate.retestTimestamp].join("|"),
  tradingDate,
  timeframe,
  session,
  side: candidate.side,
  ...
  rr: candidate.rr,
  signalIndex,
  cisdTimestamp: candidate.cisdCandle?.timestamp,
...
const summarizeCandidates = (candidates) => ({
  ...
  byTimeframe: Object.fromEntries(timeframes.map((timeframe) => [timeframe, summarizeBasic(candidates.filter((candidate) => candidate.timeframe === timeframe))])),
  bySession: Object.fromEntries(["session_open", "rth_non_open", "outside_rth"].map((session) => [session, summarizeBasic(candidates.filter((candidate) => candidate.session === session))])),
...
const oosSummary = (candidates) => {
  ...
  const sorted = candidates.slice().sort((left, right) => `${left.tradingDate}|${left.cisdTimestamp}`.localeCompare(`${right.tradingDate}|${right.cisdTimestamp}`));
  const midpoint = Math.floor(sorted.length / 2);
...
const classifyRobustness = ({ summary, rolling, oos, blockerCounts, totalEvaluated }) => {
  ...
  if ((blockerCounts["no retest of CISD open-to-close body"] ?? 0) > summary.count * 10) return "too_strict";
  return "no_edge";
};
...
async function main() {
  compileForNode();
  const { evaluateIctCisd } = await import(pathToFileURL(path.join(outRoot, "ictCisd.mjs")).href);
  ...
  for (const depth of depths) {
    const sliceSize = sliceCandlesByTimeframe[depth.timeframe] ?? 80;
    for (let index = 0; index < depth.candles.length; index += 1) {
      ...
      const candidate = evaluateIctCisd({
        candles: slice,
        sourceProvider: "mt5_read_only",
        sourceFingerprint: `mt5|${requestedSymbol}|${brokerSymbol}|${depth.timeframe}|${depth.candleCount}`,
        requestedSymbol,
        brokerSymbol,
      ...
      if (candidate.cisdCandle) diagnostics.setupConditionHits += 1;
      ...
      const outcome = simulateOutcome({ candles: depth.candles, candidate, signalIndex: index, timeframe: depth.timeframe });
      const compact = compactCandidate({ candidate, timeframe: depth.timeframe, signalIndex: index, outcome, tradingDate: parts.dayKey, session });
      if (!candidatesByKey.has(compact.key)) candidatesByKey.set(compact.key, compact);
    }
  }

  const candidates = [...candidatesByKey.values()];
  const firstTimestamp = depths.map((depth) => depth.firstTimestamp).filter(Boolean).sort()[0];
  const lastTimestamp = depths.map((depth) => depth.lastTimestamp).filter(Boolean).sort().at(-1);
  ...
  const promotionDecision =
    robustnessClassification === "paper_watchlist_candidate"
      ? "CISD may be tracked as paper-watchlist research only; Paper-Demo remains blocked until full deterministic gates pass."
      : "Do not promote CISD; keep replay-required/research-only until replay, OOS, evidence, maturity, and checklist gates improve.";

  const report = {
    status: "passed",
    diagnostic: "cisd_performance_90d",
...
main().catch((error) => {
  const report = {
    ...
    promotionDecision: "Do not promote CISD; diagnostic did not complete.",
    safety,
    authority
  };
  console.log(JSON.stringify(report, null, 2));
